...
// src/modals/searchSuggestions.ts
var import_obsidian3 = require("obsidian");

// src/modules/dataFetching.ts
var import_os = __toESM(require("os"));
var import_path = __toESM(require("path"));

// src/utils/databaseQuery.ts
var import_child_process = require("child_process");
var executeDbQuery = async (dbPath, sqlQuery) => {
  ...
  const exitCodePromise = new Promise((resolve, reject) => {
    dbQueryResult.on("close", (code, signal) => {
      if (code !== null) {
        resolve(code);
        return;
      }
      reject(new Error(`sqlite3 process terminated by signal ${signal != null ? signal : "unknown"}`));
    });
...
// src/modules/dataFetching.ts
var getBooksDbPath = () => {
  return process.env.BOOKS_DB_PATH || import_path.default.join(import_os.default.homedir(), "Library/Containers/com.apple.iBooksX/Data/Documents/BKLibrary/BKLibrary-1-091020131601.sqlite");
};
var getAnnotationsDbPath = () => {
  return process.env.ANNOTATIONS_DB_PATH || import_path.default.join(import_os.default.homedir(), "Library/Containers/com.apple.iBooksX/Data/Documents/AEAnnotation/AEAnnotation_v10312011_1727_local.sqlite");
};
var getBooks = async () => {
...
var getAnnotations = async (sortingCriterion) => {
...
var dbRequest = async (dbPath, sqlQuery) => {
  return executeDbQuery(dbPath, sqlQuery);
};
var annotationsRequest = async (dbPath, sqlQuery) => {
  return executeDbQuery(dbPath, sqlQuery);
};

// src/modules/annotationsProcessing.ts
var aggregateBooksWithAnnotations = async (sortingCriterion) => {
  const [books, annotations] = await Promise.all([getBooks(), getAnnotations(sortingCriterion)]);
  const annotationsMap = /* @__PURE__ */ new Map();
...
function enrichBooksWithAnnotations(books, annotationsMap, booksWithAnnotations) {
...
function mapAnnotationsToBooks(annotations, annotationsMap) {
...
function cleanUpTitle(title) {
  return title.replace(/[|#^[\]\\/:]+/g, "-").replace(/\s{2,}/g, " ");
}
...
var normalizeAnnotationLocation = (location) => {
  const normalizedLocation = location.slice(8, -1).replace(/\[.*?\]/g, "").replace(/\d+/g, (num) => num.padStart(4, "0"));
  return normalizedLocation;
};
var sortByLocation = (annotations) => {
...
// src/modules/templateProcessing.ts
...
var calculateAppleDate = (date) => {
  ...
  const APPLE_EPOCH_START = (/* @__PURE__ */ new Date("2001-01-01")).getTime();
...
// src/utils/manageKeepMeSection.ts
function escapeKeepMeSectionDelimiters(str) {
  return str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
var extractKeepMeSectionFromContent = (content, settings) => {
...
var saveKeepMeSectionData = async (file, data, plugin, settings) => {
  var _a;
  const isBackupFile = /-bk-\d{3,15}/i.test(file.basename);
  const isInHighlightsFolder = ((_a = file.parent) == null ? void 0 : _a.path) === settings.highlightsFolder;
...
var getKeepMeSectionDataFromSettings = (filename, settings) => {
  if (settings.keepMeSectionData && settings.keepMeSectionData[filename]) {
    return settings.keepMeSectionData[filename];
  }
  return "";
};
var embedKeepMeSectionDataIntoBookFile = (keepMeSectionData, content, settings) => {
...
// src/utils/notificationCenter.ts
var import_obsidian = require("obsidian");
var showSuccessfulImportNotice = () => {
  return new import_obsidian.Notice("Apple Books highlights imported successfully");
};
var showFailedImportNotice = (pluginName) => {
  return new import_obsidian.Notice(`[${pluginName}]:
Error importing highlights. Check console for details (\u2325 \u2318 I)`, 0);
};
var showErrorInConsole = (pluginName, error) => {
  console.error(`[${pluginName}]:`, error);
};

// src/modals/overwriteConsent.ts
var import_obsidian2 = require("obsidian");

// src/importHighlights.ts
var importHighlights = async (vault, settings, importMode = "create") => {
  ...
  if (rejected.length > 0) {
    ...
    aggregatedError.causes = rejected.map((r) => r.reason);
    throw aggregatedError;
  }
};

// src/modals/overwriteConsent.ts
var OverwriteBookModal = class extends import_obsidian2.Modal {
  constructor(app, plugin, fileDetails) {
    super(app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).addButton((YesButton) => {
      YesButton.setButtonText("Yes, overwrite").setCta().onClick(async () => {
        try {
          if (bookToOverwrite) {
...
// src/modals/searchSuggestions.ts
var IBookHighlightsPluginSuggestModal = class extends import_obsidian3.SuggestModal {
...
var IBookHighlightsPluginSearchModal = class extends IBookHighlightsPluginSuggestModal {
  constructor() {
    super(...arguments);
    this.firstRetrievedBooksWithAnnotations = null;
  }
...
// src/modules/vaultManagement.ts
var import_path2 = __toESM(require("path"));
var VaultManagement = class {
  ...
  getHighlightsFolderPath() {
    const folderPath = this.getHighlightsFolder();
    const checkPath = this.vault.getFolderByPath(folderPath);
    return checkPath;
  }
  getFilePath(filenameTemplate) {
    const filePath = import_path2.default.join(this.getHighlightsFolder(), `${filenameTemplate}.md`);
  ...
  async createBookFile(filename, content) {
    const filePath = import_path2.default.join(this.getHighlightsFolder(), `${filename}.md`);
    await this.vault.create(filePath, content);
  }
  async modifyBookFile(file, content) {
  ...
  async backupBookFile(file) {
    const backupFileName = `${file.basename}-bk-${Date.now()}.md`;
    await this.vault.adapter.rename(file.path, import_path2.default.join(this.getHighlightsFolder(), backupFileName));
  }
};
...
{{/each}}
`;
var allowedFilenameTemplateVariables = [
  "bookTitle",
  // Default
  "bookId",
  "bookAuthor",
...
var IBookHighlightsSettingTab = class extends import_obsidian4.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  addHighlightsFolderSetting(containerEl) {
    const folder = new import_obsidian4.Setting(containerEl).setName("Highlights folder").setDesc("A folder (within the root of your vault) where you want to save imported highlights").setClass("ibooks-highlights-folder");
    folder.addText(
      (text) => text.setPlaceholder("Folder to save highlights").setValue(this.plugin.settings.highlightsFolder).onChange(async (value) => {
        if (!value) {
  ...
  addImportOnStartSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Import highlights on start").setDesc("Import all highlights from all your books when Obsidian starts (requires confirmation on each start if backup is disabled)").addToggle((toggle) => {
  ...
  addBackupSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Backup highlights").setDesc(
      createFragment((el) => {
      ...
      toggle.setValue(this.plugin.settings.backup).onChange(async (value) => {
        if (!value) {
          new import_obsidian4.Notice("Disabling backups imposes a risk of data loss. Please use with caution.", 0);
        }
  ...
  addHighlightsSortingCriterionSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Highlights sorting criterion").setDesc("Sort highlights by a specific criterion. Default: By creation date (from oldest to newest)").setClass("ibooks-highlights-sorting").addDropdown((dropdown) => {
      const options = {
      ...
      dropdown.addOptions(options).setValue(this.plugin.settings.highlightsSortingCriterion).onChange(async (value) => {
        this.plugin.settings.highlightsSortingCriterion = value;
        await this.plugin.saveSettings();
      });
    });
  }
  addTemplateSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Template").setDesc("Template for highlight files").setClass("ibooks-highlights-template").addTextArea((text) => {
  ...
  addFilenameTemplateSetting(containerEl) {
    const filenameTemplate = new import_obsidian4.Setting(containerEl).setName("Template for naming highlight files").setDesc(
      createFragment((el) => {
    ...
    filenameTemplate.addTextArea((text) => {
      text.setPlaceholder("Naming template for highlight files").setValue(this.plugin.settings.filenameTemplate).onChange(async (value) => {
  ...
  addKeepMeSectionSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Template: Keep Me section").setDesc(
      createFragment((el) => {
    ...
    ).setClass("ibooks-highlights-keep-me-section").addText((text) => {
      text.setPlaceholder("Opening delimiter").setValue(this.plugin.settings.keepMeSectionOpeningDelimiter || defaultPluginSettings.keepMeSectionOpeningDelimiter).onChange(async (value) => {
      ...
      text.setPlaceholder("Closing delimiter").setValue(this.plugin.settings.keepMeSectionClosingDelimiter || defaultPluginSettings.keepMeSectionClosingDelimiter).onChange(async (value) => {
  ...
  addResetTemplateSetting(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Reset template").setDesc("Reset template to default").addButton((button) => {
...
// src/utils/backupAndImportFlow.ts
var backupAndImport = async (plugin, settings, importMode) => {
  if (settings.backup) {
    try {
...
// main.ts
var IBookHighlightsPlugin = class extends import_obsidian5.Plugin {
  async onload() {
...
function addImportOneBookCommand(plugin) {
  plugin.addCommand({
